...
'''
1. 获取页面上的cookie并解析拿到用户信息
2. 把cookie中获取到的username和password传入get_token方法拿到API访问令牌
3. 取出所有投放任务下的邀约任务并且状态为8的任务
4.遍历每一个投放任务下的状态为8的邀约任务的taskid放进返回达人邀约数的接口里面的taskId参数
5.取出返回数据里的每个邀约任务的达人数量
6.计算投放任务下的邀约任务成功发送邀约的达人数量
7.更新每个投放任务的成功发送邀约数量
'''
